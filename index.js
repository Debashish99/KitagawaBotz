...
const startHaruka = async () => {
  ...
  function title() {
    console.log(
      chalk.bold.green(
        figlet.textSync("KitagawaMarinnBotz", {
          font: "Standard",
    ...
    console.log(
      chalk.yellow(`\n${chalk.yellow("Created By Rakarmp,VynaChan")}\n`)
    );
  }
  ...
  const haruka = harukaConnect({
    ...
    browser: ["KitagawaMarinnBotz Multi Device", "Safari", "1.0.0"],
    auth: state,
  });

  require("./command/case");
  nocache("./command/case", (module) =>
    console.log(
  ...
  if (haruka.user && haruka.user.id)
    haruka.user.jid = jidNormalizedUser(haruka.user.id);
...
app.use(express.static(path.join(__dirname, 'views')))
    ...
    for (let kopel of celled) {
      if (kopel.isGroup == false) {
        if (kopel.status == "offer") {
          let nomer = await haruka.sendTextWithMentions(
            kopel.from,
            `*${haruka.user.name}* tidak bisa menerima panggilan ${
  ...
  haruka.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      for (let mek of chatUpdate.messages) {
        ...
        if (!haruka.public && !mek.key.fromMe && chatUpdate.type === "notify")
          return;
  ...
  haruka.sendContact = async (jid, kon, quoted = "", opts = {}) => {
    ...
    for (let i of kon) {
      list.push({
        ...
        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await haruka.getName(
          i + "@s.whatsapp.net"
        )}\nFN:${await haruka.getName(
          i + "@s.whatsapp.net"
  ...
  haruka.sendTextWithMentions = async (jid, text, quoted, options = {}) =>
    haruka.sendMessage(
      jid,
      {
        text: text,
  ...
  haruka.sendImage = async (jid, path, caption = "", quoted = "", options) => {
    let buffer = Buffer.isBuffer(path)
      ...
      ? fs.readFileSync(path)
      : Buffer.alloc(0);
    return await haruka.sendMessage(
      jid,
      { image: buffer, caption: caption, ...options },
      { quoted }
    );
  };
  haruka.downloadAndSaveMediaMessage = async (
    message,
    filename,
  ...
  haruka.sendText = (jid, text, quoted = "", options) =>
    haruka.sendMessage(jid, { text: text, ...options }, { quoted, ...options });

  haruka.cMod = (
    jid,
    copy,
    text = "",
    sender = haruka.user.id,
    options = {}
  ) => {
